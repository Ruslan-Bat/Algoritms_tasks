...
int AVLTree::height(Node* p) { return p ? p->height : 0; }

int AVLTree::bfactor(Node* p) { return height(p->right) - height(p->left); }
...
Node* AVLTree::findMin(Node* p) { return p->left ? findMin(p->left) : p; }
